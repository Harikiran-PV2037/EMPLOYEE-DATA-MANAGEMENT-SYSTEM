...
def request_access_grant(employee_id,status_access_control):
    ...
    while choice == '':
        ...
        if status_access_control == 'PENDING':
            ...
            print("YOUR REQUESTHAVE BEEN SUBMITTED ALREADY. PLEASE WAIT FOR THE ADMIN TO REPLY.")
            break
        if choice.upper() == 'Y':
        ...
        while access_control == 'ACCESS GRANTED':
            if user_data :
                if user_data[5] == 'ACCESS DENIED':
                    ...
                    return
                if user_data[7] == 'LOGGED OUT' and user_data[6] == 'APPROVED':
                    print(f'USERNAME {user_data[1]} - LOGGED IN SUCCESSFULLY AS {user_data[4]}\n')
...
def register():
    ...
    while True:
        try:
            ...
            print("FIRST YOU MUST ENTER THE COMPANY PROVIDED EMPLOYEE ID")
            ...
            if check_employee(emp_id)   :
                ...
                cursor.execute(''' SELECT * FROM user WHERE Emp_Id = ?''',(emp_id,))
                data = cursor.fetchone()

                if data :
                    # iF data HAS A VALUE , USER HAS ALREADY REGISTERED
                    ...
                    cursor.execute('''INSERT INTO user (Emp_id,username,password,role) VALUES(?,?,?,?)
                                        ''',(emp_id,user_name,pass_word,'EMPLOYEE'))
